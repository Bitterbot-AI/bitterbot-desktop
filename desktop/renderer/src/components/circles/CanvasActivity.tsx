...
function CardChip({ title, cardId }: { title: string; cardId: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex max-w-40 items-center rounded border bg-muted/60 px-1.5 py-px text-badge font-medium text-foreground align-baseline truncate hover:bg-muted/90"
    >
      {title || "card"}
    </button>
  );
  ...
  return (
    <div className="mx-3 mb-1 space-y-1 text-xs" data-testid="canvas-live-strip">
        ...
        >
          ...
          <div className="mt-1.5 flex items-center gap-2">
            <button
              ...
              className="rounded bg-success hover:bg-success/90 px-2 py-0.5 text-2xs font-semibold text-white"
            >
              Add it
            </button>
            <button
              ...
              className="rounded border px-2 py-0.5 text-2xs hover:bg-muted/90"
            >
      ...
      {agreements.map((s) => (
        <div
          key={`agree-${s.cardId}`}
          className="motion-enter-conversation flex items-center gap-2 rounded-lg border border-success/40 bg-success/10 px-3 py-1.5"
        >
          <Check className="h-3.5 w-3.5 shrink-0 text-success" />
          <span className="min-w-0">
            <b className="text-success dark:text-success">Everyone agrees</b> on{" "}
          ...
          <button
            ...
            className="ml-auto shrink-0 rounded bg-success hover:bg-success/90 px-2 py-0.5 text-2xs font-semibold text-white"
          >
            Lock it in
          </button>
        </div>
      ...
      {paused && part?.pauseReason && (
        <div className="motion-enter-conversation flex items-center gap-2 rounded-lg border border-warning/40 bg-warning/10 px-3 py-1.5">
          <span className="min-w-0">
            <b className="text-warning dark:text-warning">Your agent paused.</b> {part.pauseReason}
          </span>
          {budgetPaused ? (
            <button
              type="button"
              ...
              className="ml-auto shrink-0 rounded bg-warning hover:bg-warning/90 px-2 py-0.5 text-2xs font-semibold text-warning-foreground"
            >
              Give it more turns
            </button>
          ) : (
            <button
              ...
              className="ml-auto shrink-0 rounded border border-warning px-2 py-0.5 text-2xs font-medium text-warning dark:text-warning hover:bg-warning/20"
            >
              Resume
            </button>
          )}
      ...
      {stalls.map((s) => (
        <div key={`stall-${s.cardId}`} className="px-1 text-2xs text-warning dark:text-warning">
