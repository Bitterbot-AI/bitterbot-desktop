...
type EnvSnapshot = {
  ...
  bitterbotHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    bitterbotHome: process.env.BITTERBOT_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("BITTERBOT_HOME", snapshot.bitterbotHome);
  restoreKey("BITTERBOT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked BITTERBOT_HOME.
  delete process.env.BITTERBOT_HOME;
  process.env.BITTERBOT_STATE_DIR = path.join(base, ".bitterbot");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "bitterbot-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".bitterbot", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
