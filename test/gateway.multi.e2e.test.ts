...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `bitterbot-e2e-${name}-`));
  const configDir = path.join(homeDir, ".bitterbot");
    ...
    async () => {
      ...
      const [hookResA, hookResB] = await Promise.all([
        postJson(
          ...
          { "x-bitterbot-token": gwA.hookToken },
        ),
        postJson(
          ...
          { "x-bitterbot-token": gwB.hookToken },
        ),
      ]);
