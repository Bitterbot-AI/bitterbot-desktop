...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("bitterbot-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("bitterbot-bin-sherpa-");
      const modelDir = await makeTempDir("bitterbot-sherpa-model-");
      ...
      const cfg: BitterbotConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("bitterbot-bin-whispercpp-");
      const modelDir = await makeTempDir("bitterbot-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("bitterbot-bin-gemini-");
      ...
      const cfg: BitterbotConfig = { tools: { media: { image: {} } } };
