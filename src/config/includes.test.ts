...
const ETC_BITTERBOT_DIR = path.join(ROOT_DIR, "etc", "bitterbot");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "bitterbot.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcBitterbotPath(...parts: string[]) {
  return path.join(ETC_BITTERBOT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcBitterbotPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "bitterbot.json"))).toEqual({
      shared: true,
    });
  });
});
