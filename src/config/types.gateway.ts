...
export type WideAreaDiscoveryConfig = {
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "bitterbot.internal"). */
  domain?: string;
};
...
export type CanvasHostConfig = {
  enabled?: boolean;
  /** Directory to serve (default: ~/.bitterbot/workspace/canvas). */
  root?: string;
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/bitterbot"). */
  basePath?: string;
 ...
 * Used when Bitterbot runs behind an identity-aware proxy (Pomerium, Caddy + OAuth, etc.)
...
export type GatewayConfig = {
  /** Single multiplexed port for Gateway WS + HTTP (default: 19001). */
  port?: number;
  /**
