...
import type { BitterbotConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: BitterbotConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: BitterbotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = BitterbotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as BitterbotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as BitterbotConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as BitterbotConfig,
  };
}
