...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-pruning-suite-"));
});
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.BITTERBOT_SESSION_CACHE_TTL_MS;
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.BITTERBOT_SESSION_CACHE_TTL_MS;
    } else {
      process.env.BITTERBOT_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
