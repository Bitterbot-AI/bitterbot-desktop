...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-bitterbot",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-bitterbot#general");
  });
  ...
  it("derives session transcripts dir from BITTERBOT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { BITTERBOT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.BITTERBOT_STATE_DIR = "/custom/state";
    try {
