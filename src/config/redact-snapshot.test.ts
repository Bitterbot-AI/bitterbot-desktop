...
import type { ConfigFileSnapshot } from "./types.bitterbot.js";
import {
  REDACTED_SENTINEL,
...
function makeSnapshot(config: Record<string, unknown>, raw?: string): ConfigFileSnapshot {
  return {
    path: "/home/user/.bitterbot/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/bitterbot/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/bitterbot/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/bitterbot/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/bitterbot/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = BitterbotSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "BitterbotConfig";
    const hints = mapSensitivePaths(BitterbotSchema, "", {});
