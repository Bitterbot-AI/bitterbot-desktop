...
export type ToolPolicyConfig = {
   ...
   *
   * Intended for additive configuration (e.g., "also allow bitter") without forcing
...
export type MemorySearchConfig = {
  ...
  query?: {
    ...
    hybrid?: {
      ...
      /**
       * How to merge vector and keyword search results.
       * - `"rrf"` (default): Reciprocal Rank Fusion — rank-based fusion that
       *   doesn't require comparable score scales. Importance is decoupled from
       *   retrieval and used only for memory lifecycle.
       * - `"weighted"`: Legacy weighted average (vectorWeight * v + textWeight * k)
       *   with importance boost applied post-merge.
       */
      mergeStrategy?: "rrf" | "weighted";
    };
    /**
     * Weight for importance-score boost applied after hybrid merge (0–1).
     * Only active when `hybrid.mergeStrategy` is `"weighted"`.
     * With RRF (default), importance is decoupled from retrieval entirely.
     * Default: 0.15.
     */
    importanceWeight?: number;
    /** Recency boost for search results (post-RRF multiplier). */
    recency?: {
...
export type ToolsConfig = {
  ...
  /** Crypto wallet configuration (Coinbase AgentKit on Base). */
  wallet?: WalletConfig;
};
