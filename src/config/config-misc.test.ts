...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = BitterbotSchema.safeParse({
      $schema: "https://bitterbot.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://bitterbot.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = BitterbotSchema.safeParse({ $schema: 123 });
...
describe("ui.seamColor", () => {
  ...
  it("rejects non-hex colors", () => {
    const res = validateConfigObject({ ui: { seamColor: "bitterbot" } });
