...
describe("shared/frontmatter", () => {
  ...
  test("resolveBitterbotManifestBlock parses JSON5 metadata and picks bitterbot block", () => {
    const frontmatter = {
      metadata: "{ bitterbot: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveBitterbotManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveBitterbotManifestBlock returns undefined for invalid input", () => {
    expect(resolveBitterbotManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveBitterbotManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveBitterbotManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
