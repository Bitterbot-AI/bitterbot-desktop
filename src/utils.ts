...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".bitterbot");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.BITTERBOT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$BITTERBOT_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via BITTERBOT_STATE_DIR.
