...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.bitterbotCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.bitterbotCanvasA2UIAction &&
        typeof window.bitterbotCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.bitterbotSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("bitterbot:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Bitterbot node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.bitterbotSendUserAction === "function"
        ? window.bitterbotSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.BITTERBOT_SKIP_CANVAS_HOST)) {
    return true;
  }
...
export async function createCanvasHostHandler(
  ...
  const handleHttpRequest = async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      let opened: Awaited<ReturnType<typeof resolveFileWithinRoot>>;
      try {
        opened = await resolveFileWithinRoot(rootReal, urlPath);
      } catch (resolveErr) {
        console.error(
          `[canvas] resolveFileWithinRoot error: rootReal=${rootReal} urlPath=${urlPath} err=${String(resolveErr)}`,
        );
        opened = null;
      }
      if (!opened) {
        console.warn(`[canvas] file not found: rootReal=${rootReal} urlPath=${urlPath}`);
        if (urlPath === "/" || urlPath.endsWith("/")) {
          res.statusCode = 404;
          res.setHeader("Content-Type", "text/html; charset=utf-8");
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Bitterbot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
