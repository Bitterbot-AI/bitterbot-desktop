...
describe("canvas host state dir defaults", () => {
  ...
  it("uses BITTERBOT_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Bitterbot Canvas");
    } finally {
      await handler.close();
