...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "bitterbot-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    BITTERBOT_GATEWAY_TOKEN: "test-token",
    BITTERBOT_CONFIG_DIR: join(sandbox.rootDir, "config"),
    BITTERBOT_WORKSPACE_DIR: join(sandbox.rootDir, "bitterbot"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        BITTERBOT_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        BITTERBOT_EXTRA_MOUNTS: undefined,
        BITTERBOT_HOME_VOLUME: "bitterbot-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("BITTERBOT_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("BITTERBOT_EXTRA_MOUNTS=");
    expect(envFile).toContain("BITTERBOT_HOME_VOLUME=bitterbot-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("bitterbot-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("bitterbot-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg BITTERBOT_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
