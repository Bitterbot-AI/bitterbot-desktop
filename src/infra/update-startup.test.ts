...
describe("update-startup", () => {
  ...
  let resolveBitterbotPackageRoot: (typeof import("./bitterbot-root.js"))["resolveBitterbotPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "BITTERBOT_STATE_DIR");
    prevStateDir = process.env.BITTERBOT_STATE_DIR;
    process.env.BITTERBOT_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveBitterbotPackageRoot } = await import("./bitterbot-root.js"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    vi.mocked(resolveBitterbotPackageRoot).mockResolvedValue("/opt/bitterbot");
