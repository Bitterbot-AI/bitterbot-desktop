...
const DEFAULT_PACKAGE_NAME = "bitterbot";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "bitterbot.mjs");
    ...
    const doctorStep = await runStep(
      step("bitterbot doctor", doctorArgv, gitRoot, { BITTERBOT_UPDATE_IN_PROGRESS: "1" }),
    );
