...
function candidateBinDirs(opts: EnsureBitterbotPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `bitterbot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "bitterbot");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.BITTERBOT_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "bitterbot"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `bitterbot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureBitterbotCliOnPath(opts: EnsureBitterbotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.BITTERBOT_PATH_BOOTSTRAPPED)) {
    return;
  }
