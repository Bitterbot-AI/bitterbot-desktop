...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.BITTERBOT_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Bitterbot. You can keep running the web gateway without it: \`${formatCliCommand("bitterbot gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
