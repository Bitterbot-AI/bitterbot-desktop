...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: BitterbotConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: BitterbotConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: BitterbotConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: BitterbotConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: BitterbotConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: BitterbotConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: BitterbotConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
