...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      domain: "bitterbot.internal.",
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Mac Studio (Bitterbot)",
      tailnetIPv4: "100.123.224.76",
      tailnetIPv6: "fd7a:115c:a1e0::8801:e04c",
      hostLabel: "studio-london",
      instanceLabel: "studio-london",
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/bitterbot",
    });

    expect(txt).toContain(`$ORIGIN bitterbot.internal.`);
    ...
    expect(txt).toContain(`_bitterbot-gw._tcp IN PTR studio-london._bitterbot-gw._tcp`);
    expect(txt).toContain(`studio-london._bitterbot-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Bitterbot)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/bitterbot`);
  });
