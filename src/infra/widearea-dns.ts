...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*bitterbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "bitterbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "bitterbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "bitterbot-gw");
  ...
  records.push(`_bitterbot-gw._tcp IN PTR ${instanceLabel}._bitterbot-gw._tcp`);
  records.push(`${instanceLabel}._bitterbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._bitterbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; bitterbot-content-hash: ${contentHash}\n${contentBody}`;
}
