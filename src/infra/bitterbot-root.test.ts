...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__bitterbot_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "bitterbot-root");
...
describe("resolveBitterbotPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveBitterbotPackageRootSync } = await import("./bitterbot-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "bitterbot");
    const pkgRoot = path.join(project, "node_modules", "bitterbot");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "bitterbot" }));

    expect(resolveBitterbotPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "bitterbot");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "bitterbot.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "bitterbot" }));

    expect(resolveBitterbotPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveBitterbotPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-bitterbot package roots", async () => {
    const { resolveBitterbotPackageRootSync } = await import("./bitterbot-root.js");

    const pkgRoot = fx("not-bitterbot");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-bitterbot" }));

    expect(resolveBitterbotPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveBitterbotPackageRoot } = await import("./bitterbot-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "bitterbot" }));

    await expect(resolveBitterbotPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
