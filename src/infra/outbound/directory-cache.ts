...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: BitterbotConfig | null = null;
  ...
  get(key: string, cfg: BitterbotConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: BitterbotConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: BitterbotConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: BitterbotConfig): void {
