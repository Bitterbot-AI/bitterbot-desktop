...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.BITTERBOT_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.BITTERBOT_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.BITTERBOT_TEST_TAILSCALE_BINARY;
    } else {
      process.env.BITTERBOT_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
