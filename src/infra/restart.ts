...
export function triggerBitterbotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.BITTERBOT_SYSTEMD_UNIT,
        process.env.BITTERBOT_PROFILE,
      );
  ...
  const label =
    process.env.BITTERBOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.BITTERBOT_PROFILE);
