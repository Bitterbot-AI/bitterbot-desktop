...
export const POSIX_BITTERBOT_TMP_DIR = "/tmp/bitterbot";

type ResolvePreferredBitterbotTmpDirOptions = {
...
export function resolvePreferredBitterbotTmpDir(
  options: ResolvePreferredBitterbotTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "bitterbot" : `bitterbot-${uid}`;
  ...
  try {
    ...
    accessSync(POSIX_BITTERBOT_TMP_DIR, fs.constants.W_OK | fs.constants.X_OK);
  ...
  try {
    ...
    mkdirSync(POSIX_BITTERBOT_TMP_DIR, { recursive: true, mode: 0o700 });
    try {
      const preferred = lstatSync(POSIX_BITTERBOT_TMP_DIR);
      if (!preferred.isDirectory() || preferred.isSymbolicLink()) {
        return fallback();
