...
let ensureBitterbotCliOnPath: typeof import("./path-env.js").ensureBitterbotCliOnPath;

describe("ensureBitterbotCliOnPath", () => {
  const envKeys = [
    "PATH",
    "BITTERBOT_PATH_BOOTSTRAPPED",
    "BITTERBOT_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureBitterbotCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling bitterbot exists", () => {
    const tmp = abs("/tmp/bitterbot-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "bitterbot");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.BITTERBOT_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.BITTERBOT_PATH_BOOTSTRAPPED = "1";
    ensureBitterbotCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/bitterbot-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "bitterbot");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/bitterbot-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "bitterbot");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "bitterbot");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/bitterbot-path/case-linuxbrew");
