...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.BITTERBOT_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.BITTERBOT_STATE_DIR;
  const configPath = env.BITTERBOT_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    BITTERBOT_PROFILE: profile,
    BITTERBOT_STATE_DIR: stateDir,
    BITTERBOT_CONFIG_PATH: configPath,
    BITTERBOT_GATEWAY_PORT: String(port),
    BITTERBOT_GATEWAY_TOKEN: token,
    BITTERBOT_LAUNCHD_LABEL: resolvedLaunchdLabel,
    BITTERBOT_SYSTEMD_UNIT: systemdUnit,
    BITTERBOT_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    BITTERBOT_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    BITTERBOT_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    BITTERBOT_STATE_DIR: stateDir,
    BITTERBOT_CONFIG_PATH: configPath,
    BITTERBOT_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    BITTERBOT_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    BITTERBOT_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    BITTERBOT_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    BITTERBOT_LOG_PREFIX: "node",
    BITTERBOT_SERVICE_MARKER: NODE_SERVICE_MARKER,
    BITTERBOT_SERVICE_KIND: NODE_SERVICE_KIND,
    BITTERBOT_SERVICE_VERSION: VERSION,
  };
}
