...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.bitterbot.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", BITTERBOT_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    const env: Record<string, string | undefined> = {
      HOME: "/Users/test",
      BITTERBOT_PROFILE: "default",
    };
    const repair = await repairLaunchAgentBootstrap({ env });
    expect(repair.ok).toBe(true);

    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const label = "ai.bitterbot.gateway";
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when BITTERBOT_PROFILE is default", () => {
    const env = { HOME: "/Users/test", BITTERBOT_PROFILE: "default" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.bitterbot.gateway.plist",
    );
  });

  it("uses default label when BITTERBOT_PROFILE is unset", () => {
  ...
  it("uses profile-specific label when BITTERBOT_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", BITTERBOT_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.bitterbot.jbphoenix.plist",
    );
  });

  it("prefers BITTERBOT_LAUNCHD_LABEL over BITTERBOT_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      BITTERBOT_PROFILE: "jbphoenix",
      BITTERBOT_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from BITTERBOT_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      BITTERBOT_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty BITTERBOT_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      BITTERBOT_PROFILE: "myprofile",
      BITTERBOT_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.bitterbot.myprofile.plist",
    );
  });
  ...
  it("trims whitespace from BITTERBOT_PROFILE", () => {
    const env = { HOME: "/Users/test", BITTERBOT_PROFILE: "  myprofile  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.bitterbot.myprofile.plist",
    );
  });
