...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.BITTERBOT_GATEWAY_PORT).toBe("18789");
    expect(env.BITTERBOT_GATEWAY_TOKEN).toBe("secret");
    expect(env.BITTERBOT_SERVICE_MARKER).toBe("bitterbot");
    expect(env.BITTERBOT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.BITTERBOT_SERVICE_VERSION).toBe("string");
    expect(env.BITTERBOT_SYSTEMD_UNIT).toBe("bitterbot-gateway.service");
    if (process.platform === "darwin") {
      expect(env.BITTERBOT_LAUNCHD_LABEL).toBe("ai.bitterbot.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", BITTERBOT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.BITTERBOT_SYSTEMD_UNIT).toBe("bitterbot-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.BITTERBOT_LAUNCHD_LABEL).toBe("ai.bitterbot.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".bitterbot"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", BITTERBOT_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".bitterbot-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", BITTERBOT_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".bitterbot"));
  });

  it("uses BITTERBOT_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", BITTERBOT_STATE_DIR: "/var/lib/bitterbot" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/bitterbot"));
  });

  it("expands ~ in BITTERBOT_STATE_DIR", () => {
    const env = { HOME: "/Users/test", BITTERBOT_STATE_DIR: "~/bitterbot-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/bitterbot-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { BITTERBOT_STATE_DIR: "C:\\State\\bitterbot" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\bitterbot");
  });
});
