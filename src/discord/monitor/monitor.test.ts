...
describe("agent components", () => {
  const createCfg = (): BitterbotConfig => ({}) as BitterbotConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): BitterbotConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockReset().mockReturnValue("/tmp/bitterbot-sessions-test.json");
  });
