...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "bitterbot",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/bitterbot.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("bitterbot");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "bitterbot");
