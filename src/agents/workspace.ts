...
export function resolveDefaultAgentWorkspaceDir(
  ...
  return path.join(home, ".bitterbot", "workspace");
}
...
export const DEFAULT_GENOME_FILENAME = "GENOME.md";
export const DEFAULT_PROTOCOLS_FILENAME = "PROTOCOLS.md";
const WORKSPACE_STATE_DIRNAME = ".bitterbot";
...
export type WorkspaceBootstrapFileName =
  ...
  | typeof DEFAULT_GENOME_FILENAME
  | typeof DEFAULT_PROTOCOLS_FILENAME;
...
export async function ensureAgentWorkspace(params?: {
  ...
  const genomePath = path.join(dir, DEFAULT_GENOME_FILENAME);
  const protocolsPath = path.join(dir, DEFAULT_PROTOCOLS_FILENAME);
  const toolsPath = path.join(dir, DEFAULT_TOOLS_FILENAME);
  const heartbeatPath = path.join(dir, DEFAULT_HEARTBEAT_FILENAME);
  const memoryPath = path.join(dir, DEFAULT_MEMORY_FILENAME);

  const isBrandNewWorkspace = await (async () => {
    const paths = [genomePath, protocolsPath, toolsPath, heartbeatPath];
  ...
  // Biological identity files
  await writeFileIfMissing(genomePath, await loadTemplate(DEFAULT_GENOME_FILENAME));
  await writeFileIfMissing(protocolsPath, await loadTemplate(DEFAULT_PROTOCOLS_FILENAME));
...
const DEFAULT_SCRATCH_FILENAME = "memory/scratch.md";
...
async function resolveMemoryBootstrapEntries(
  ...
  // Also discover memory/scratch.md (Write-Ahead Log) for bootstrap injection
  const scratchPath = path.join(resolvedDir, DEFAULT_SCRATCH_FILENAME);
  try {
    await fs.access(scratchPath);
    const scratchContent = await fs.readFile(scratchPath, "utf-8");
    // Only include if it has actual notes (not just the header)
    if (scratchContent.includes("- [")) {
      entries.push({
        name: DEFAULT_MEMORY_FILENAME, // Loaded as MEMORY.md type for bootstrap inclusion
...
export async function loadWorkspaceBootstrapFiles(dir: string): Promise<WorkspaceBootstrapFile[]> {
  ...
  const entries: Array<{
    ...
    {
      name: DEFAULT_GENOME_FILENAME,
      filePath: path.join(resolvedDir, DEFAULT_GENOME_FILENAME),
    },
    {
      name: DEFAULT_PROTOCOLS_FILENAME,
      filePath: path.join(resolvedDir, DEFAULT_PROTOCOLS_FILENAME),
    },
    {
