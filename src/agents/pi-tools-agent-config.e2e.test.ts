...
describe("Agent-specific tool filtering", () => {
  ...
  it("defaults apply_patch to workspace-only (blocks traversal)", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-pi-tools-"));
...
*** End Patch`;
    ...
    const cfg: BitterbotConfig = {
      ...
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/bitterbot-restricted",
            tools: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createBitterbotCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createBitterbotCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createBitterbotCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createBitterbotCodingTools({
      config: cfg,
    ...
    const bobTools = createBitterbotCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createBitterbotCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createBitterbotCodingTools({
      config: cfg,
