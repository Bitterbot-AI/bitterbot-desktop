...
import { __testing, createBitterbotCodingTools } from "./pi-tools.js";
...
describe("createBitterbotCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createBitterbotTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createBitterbotCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "bitterbot-outside.txt");
