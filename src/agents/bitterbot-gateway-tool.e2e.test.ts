...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const envSnapshot = captureEnv(["BITTERBOT_STATE_DIR", "BITTERBOT_PROFILE"]);
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-test-"));
    process.env.BITTERBOT_STATE_DIR = stateDir;
    process.env.BITTERBOT_PROFILE = "isolated";

    try {
      const tool = createBitterbotTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: bitterbot --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/bitterbot" } }\n}\n';
