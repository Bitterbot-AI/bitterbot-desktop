...
type WebFetchConfig = NonNullable<BitterbotConfig["tools"]>["web"] extends infer Web
...
function resolveFetchConfig(cfg?: BitterbotConfig): WebFetchConfig {
...
async function runWebFetch(params: WebFetchRuntimeParams): Promise<Record<string, unknown>> {
  ...
  try {
    if (!res.ok) {
      // x402 / L402: surface payment requirements as structured data so the
      // agent can chain into the wallet pay_for_resource tool.
      if (res.status === 402) {
        const authHeader = res.headers.get("www-authenticate") ?? "";
        const paymentAmount = res.headers.get("x-payment-amount") ?? "";
        const paymentToken = res.headers.get("x-payment-token") ?? "";
        const paymentNetwork = res.headers.get("x-payment-network") ?? "";
        const paymentAddress = res.headers.get("x-payment-address") ?? "";

        const bodyResult = await readResponseText(res, { maxBytes: DEFAULT_ERROR_MAX_BYTES });
        const bodyText = bodyResult.text;
        // Try to parse a JSON body for structured payment details
        let bodyPaymentDetails: Record<string, unknown> | undefined;
        try {
          const parsed = JSON.parse(bodyText);
          if (parsed && typeof parsed === "object") {
            bodyPaymentDetails = parsed as Record<string, unknown>;
          }
        } catch {
          // Not JSON — that's fine, include as snippet
        }
        ...
        return {
          ...
          instruction:
            "This resource requires payment. Follow the 'Handling Paywalls' workflow in your system prompt to pay for and retrieve this content.",
          x402_headers: {
          ...
          body_snippet: bodyText.substring(0, 1000),
          fetchedAt: new Date().toISOString(),
          tookMs: Date.now() - start,
        };
      }
