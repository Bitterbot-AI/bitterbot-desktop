...
type WebSearchConfig = NonNullable<BitterbotConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: BitterbotConfig): WebSearchConfig {
...
function resolveTavilySearchDepth(tavily?: TavilyConfig): string {
  const fromConfig =
    tavily && "searchDepth" in tavily && typeof tavily.searchDepth === "string"
      ? tavily.searchDepth.trim()
      : "";
  return fromConfig || DEFAULT_TAVILY_SEARCH_DEPTH;
}
    ...
    params.provider === "brave"
      ...
      : params.provider === "perplexity"
        ...
        : params.provider === "tavily"
          ? `${params.provider}:${params.query}:${params.count}:${params.tavilySearchDepth ?? DEFAULT_TAVILY_SEARCH_DEPTH}`
  ...
  if (params.provider === "tavily") {
    ...
    const mapped = results.map((entry) => {
      ...
      return {
        ...
        url,
        content: content ? wrapWebContent(content, "web_search") : "",
        score: entry.score,
