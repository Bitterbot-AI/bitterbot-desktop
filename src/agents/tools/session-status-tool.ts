...
export function createSessionStatusTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      // Append tool cache and compression stats
      const cacheStats = (() => {
        try {
      ...
      const compressionStats = (() => {
        const stored = getTruncatedOriginalsSize();
        return stored > 0 ? `\n📦 Compression: ${stored} truncated originals stored` : "";
      })();
      const fullStatus = statusText + cacheStats + compressionStats;

      return {
        content: [{ type: "text", text: fullStatus }],
        details: {
          ok: true,
          sessionKey: resolved.key,
          changedModel,
