...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("bitterbot-ws-", async (workspaceDir) => {
      await withTempDir("bitterbot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createBitterbotCodingTools({ workspaceDir });
    ...
    await withTempDir("bitterbot-ws-", async (workspaceDir) => {
      await withTempDir("bitterbot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello bitterbot");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("bitterbot-ws-", async (workspaceDir) => {
      await withTempDir("bitterbot-override-", async (overrideDir) => {
        const tools = createBitterbotCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("bitterbot-sandbox-", async (sandboxDir) => {
      await withTempDir("bitterbot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "bitterbot-sbx-test",
        ...
        const tools = createBitterbotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
