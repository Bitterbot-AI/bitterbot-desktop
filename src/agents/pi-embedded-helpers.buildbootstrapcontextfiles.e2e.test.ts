...
describe("buildBootstrapContextFiles", () => {
  it("keeps missing markers", () => {
    ...
    expect(buildBootstrapContextFiles(files)).toEqual([
      {
        path: "/tmp/PROTOCOLS.md",
        content: "[MISSING] Expected at: /tmp/PROTOCOLS.md",
      },
    ]);
  });
  ...
  it("keeps content under the default limit", () => {
    ...
    expect(result?.content).not.toContain("[...truncated, read PROTOCOLS.md for full content...]");
  });
  ...
  it("enforces strict total cap even when truncation markers are present", () => {
    const files = [
      makeFile({ name: "PROTOCOLS.md", content: "a".repeat(1_000) }),
      makeFile({ name: "TOOLS.md", path: "/tmp/TOOLS.md", content: "b".repeat(1_000) }),
    ];
    const result = buildBootstrapContextFiles(files, {
      maxChars: 100,
      totalMaxChars: 150,
  ...
  it("skips bootstrap injection when remaining total budget is too small", () => {
    const files = [makeFile({ name: "PROTOCOLS.md", content: "a".repeat(1_000) })];
    const result = buildBootstrapContextFiles(files, {
      maxChars: 200,
      totalMaxChars: 40,
    });
