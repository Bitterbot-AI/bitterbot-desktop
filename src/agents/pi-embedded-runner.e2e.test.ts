...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-embedded-agent-"));
...
const ensureModels = (cfg: BitterbotConfig) => ensureBitterbotModelsJson(cfg, agentDir) as unknown;
