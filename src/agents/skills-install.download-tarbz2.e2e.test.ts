...
const originalBitterbotStateDir = process.env.BITTERBOT_STATE_DIR;

afterEach(() => {
  if (originalBitterbotStateDir === undefined) {
    delete process.env.BITTERBOT_STATE_DIR;
  } else {
    process.env.BITTERBOT_STATE_DIR = originalBitterbotStateDir;
  }
});
...
describe("installSkill download extraction safety (tar.bz2)", () => {
  ...
  it("rejects tar.bz2 traversal before extraction", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-skills-install-"));
    try {
