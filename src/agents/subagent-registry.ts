...
export function registerSubagentRun(params: {
  ...
  // Emit a lifecycle event for the child run so the frontend cowork panel
  // can display it as a sub-task.  Include the label and a parentTaskId
  // that maps to the requester's session (the parent agent run).
  emitAgentEvent({
    runId: params.runId,
    stream: "lifecycle",
    data: {
      phase: "start",
      label: params.label || params.task,
      parentTaskId: params.requesterSessionKey,
      startedAt: now,
    },
  });
