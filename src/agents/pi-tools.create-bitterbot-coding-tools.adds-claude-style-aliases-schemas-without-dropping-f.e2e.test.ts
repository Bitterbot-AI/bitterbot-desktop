...
describe("createBitterbotCodingTools", () => {
  it("uses workspaceDir for Read tool path resolution", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-ws-"));
    try {
      ...
      const tools = createBitterbotCodingTools({ workspaceDir: tmpDir });
  ...
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-alias-"));
    try {
      const tools = createBitterbotCodingTools({ workspaceDir: tmpDir });
      const readTool = tools.find((tool) => tool.name === "read");
      const writeTool = tools.find((tool) => tool.name === "write");
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-structured-write-"));
    try {
      const tools = createBitterbotCodingTools({ workspaceDir: tmpDir });
      const writeTool = tools.find((tool) => tool.name === "write");
      expect(writeTool).toBeDefined();
      ...
      await writeTool?.execute("tool-structured-write", {
        ...
        content: [
          ...
          { type: "input_text", text: "const root = path.join(process.env.HOME, 'bitterbot');\n" },
        ],
      });

      const written = await fs.readFile(path.join(tmpDir, "structured-write.js"), "utf8");
      expect(written).toBe(
        "const path = require('path');\nconst root = path.join(process.env.HOME, 'bitterbot');\n",
      );
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-structured-edit-"));
    try {
