...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "bitterbot.sandbox=1");
  args.push("--label", `bitterbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `bitterbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `bitterbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "bitterbot.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`bitterbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`bitterbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("bitterbot sandbox recreate --all");
}
