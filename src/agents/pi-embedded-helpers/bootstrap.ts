...
export function resolveBootstrapMaxChars(cfg?: BitterbotConfig): number {
...
export function resolveBootstrapTotalMaxChars(cfg?: BitterbotConfig): number {
...
export function buildBootstrapContextFiles(
  ...
  const memoryBudgetChars = resolveMemoryMdBudgetChars(opts?.contextWindowTokens);
  ...
  for (const file of files) {
    ...
    // Adaptive sizing for MEMORY.md and scratch files
    const isScratchFile = file.path.endsWith("scratch.md");
    const isMemoryFile = file.name === "MEMORY.md" || file.name === "memory.md";
    let effectiveMaxChars = Math.max(1, Math.min(maxChars, remainingTotalChars));

    if (isMemoryFile && !isScratchFile) {
      // Apply adaptive MEMORY.md budget based on model context window
      effectiveMaxChars = Math.max(1, Math.min(memoryBudgetChars, remainingTotalChars));
    }

    let content = file.content ?? "";

    // Inject scratch.md with special header
    if (isScratchFile) {
    ...
    remainingTotalChars = Math.max(0, remainingTotalChars - finalContent.length);
    result.push({
      path: file.path,
