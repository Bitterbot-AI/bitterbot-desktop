...
describe("createBitterbotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from bitterbot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "bitterbot-sandbox");
    const sandbox = {
      enabled: true,
      sessionKey: "sandbox:test",
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "bitterbot-workspace"),
    ...
    const tools = createBitterbotCodingTools({ sandbox });
