...
import { resolveBitterbotPackageRootSync } from "../../infra/bitterbot-root.js";
...
export function resolveBundledSkillsDir(
  ...
  const override = process.env.BITTERBOT_BUNDLED_SKILLS_DIR?.trim();
  if (override) {
    return override;
  }
  ...
  try {
    ...
    const packageRoot = resolveBitterbotPackageRootSync({
      argv1,
      moduleUrl,
      cwd,
    });
