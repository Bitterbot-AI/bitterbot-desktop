...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "bitterbot-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "bitterbot-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "bitterbot-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "bitterbot-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.bitterbot when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "bitterbot-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers BITTERBOT_HOME over HOME", () => {
    vi.stubEnv("BITTERBOT_HOME", "/srv/bitterbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/bitterbot-home"));
...
describe("shortenHomePath", () => {
  it("uses $BITTERBOT_HOME prefix when BITTERBOT_HOME is set", () => {
    vi.stubEnv("BITTERBOT_HOME", "/srv/bitterbot-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("shortenHomeInString", () => {
  it("uses $BITTERBOT_HOME replacement when BITTERBOT_HOME is set", () => {
    vi.stubEnv("BITTERBOT_HOME", "/srv/bitterbot-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/bitterbot")).toBe(path.resolve(os.homedir(), "bitterbot"));
  });
  ...
  it("prefers BITTERBOT_HOME for tilde expansion", () => {
    vi.stubEnv("BITTERBOT_HOME", "/srv/bitterbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/bitterbot")).toBe(path.resolve("/srv/bitterbot-home", "bitterbot"));
