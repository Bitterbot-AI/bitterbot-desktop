...
export const normalizePluginsConfig = (
  config?: BitterbotConfig["plugins"],
  ...
  return {
    ...
    slots: {
      memory: memorySlot === undefined ? defaultSlotIdForKey("memory") ?? null : memorySlot,
    },
    entries: normalizePluginEntries(config?.entries),
  };
};

const hasExplicitMemorySlot = (plugins?: BitterbotConfig["plugins"]) =>
  Boolean(plugins?.slots && Object.prototype.hasOwnProperty.call(plugins.slots, "memory"));

const hasExplicitMemoryEntry = (_plugins?: BitterbotConfig["plugins"]) => false;

const hasExplicitPluginConfig = (plugins?: BitterbotConfig["plugins"]) => {
  if (!plugins) {
    return false;
  }
