...
import { loadBitterbotPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `bitterbot-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.BITTERBOT_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "bitterbot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.BITTERBOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadBitterbotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.BITTERBOT_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadBitterbotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadBitterbotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
