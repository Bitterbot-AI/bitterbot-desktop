...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: BitterbotPluginDefinition;
  register?: BitterbotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as BitterbotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as BitterbotPluginDefinition;
...
export function loadBitterbotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverBitterbotPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "bitterbot/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "bitterbot/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: BitterbotPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as BitterbotPluginModule;
    } catch (err) {
