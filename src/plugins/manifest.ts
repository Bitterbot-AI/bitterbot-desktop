...
// package.json "bitterbot" metadata (used for onboarding/catalog)
...
export type BitterbotPackageManifest = {
  extensions?: string[];
...
): BitterbotPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
  // Check current key first, then legacy keys for backwards compat.
