...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `bitterbot-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "bitterbot.plugin.json"), JSON.stringify(manifest), "utf-8");
}
