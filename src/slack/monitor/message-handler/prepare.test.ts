...
describe("slack prepareSlackMessage inbound contract", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bitterbot-slack-thread-"));
  });
  ...
  function createThreadSlackCtx(params: { cfg: BitterbotConfig; replies: unknown }) {
...
describe("prepareSlackMessage sender prefix", () => {
  it("prefixes channel bodies with sender label", async () => {
    const ctx = {
      cfg: {
        agents: { defaults: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/bitterbot" } },
        channels: { slack: {} },
      },
      accountId: "default",
      botToken: "xoxb",
      ...
      threadInheritParent: false,
      slashCommand: { command: ".bitterbot", enabled: true },
      textLimit: 2000,
