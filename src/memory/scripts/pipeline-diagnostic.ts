...
const highImportance = queryOne<{ c: number }>(
  "SELECT COUNT(*) as c FROM chunks WHERE importance_score >= 0.8"
)?.c ?? 0;
const medImportance = queryOne<{ c: number }>(
  "SELECT COUNT(*) as c FROM chunks WHERE importance_score >= 0.3 AND importance_score < 0.8"
)?.c ?? 0;
const lowImportance = queryOne<{ c: number }>(
  "SELECT COUNT(*) as c FROM chunks WHERE importance_score < 0.3"
)?.c ?? 0;
...
if (insightCount > 0) {
  const recentInsights = query<{ content: string; mode: string; confidence: number; created_at: number }>(
  ...
  for (const i of recentInsights) {
    ...
    console.log(`  [${date}] (${i.mode}, conf=${i.confidence.toFixed(2)}) ${i.content.slice(0, 120)}`);
  }
}
...
try {
  const targetCount = queryOne<{ c: number }>(
    "SELECT COUNT(*) as c FROM curiosity_targets"
  )?.c ?? 0;
  console.log(`Total curiosity targets: ${targetCount}`);

  const unresolvedTargets = query<{ id: string; description: string; priority: number; created_at: number; resolved_at: number | null }>(
  ...
  const regionCount = queryOne<{ c: number }>(
    "SELECT COUNT(*) as c FROM curiosity_regions"
  )?.c ?? 0;
  console.log(`\nKnowledge regions: ${regionCount}`);

  const assessmentCount = queryOne<{ c: number }>(
    "SELECT COUNT(*) as c FROM curiosity_assessments"
  )?.c ?? 0;
...
const archived = queryOne<{ c: number }>(
  "SELECT COUNT(*) as c FROM chunks WHERE lifecycle = 'archived'"
)?.c ?? 0;
const expired = queryOne<{ c: number }>(
  "SELECT COUNT(*) as c FROM chunks WHERE lifecycle = 'expired'"
)?.c ?? 0;
const consolidated = queryOne<{ c: number }>(
  "SELECT COUNT(*) as c FROM chunks WHERE lifecycle = 'consolidated'"
)?.c ?? 0;
const frozen = queryOne<{ c: number }>(
  "SELECT COUNT(*) as c FROM chunks WHERE lifecycle = 'frozen'"
)?.c ?? 0;
console.log(`Archived: ${archived} | Expired: ${expired} | Consolidated: ${consolidated} | Frozen: ${frozen}`);

// Check audit log
try {
  const auditCount = queryOne<{ c: number }>(
    "SELECT COUNT(*) as c FROM consolidation_audit_log"
  )?.c ?? 0;
...
try {
  const execCount = queryOne<{ c: number }>(
    "SELECT COUNT(*) as c FROM skill_executions"
  )?.c ?? 0;
  ...
  if (execCount > 0) {
    ...
    for (const e of execStats) {
      console.log(`  Skill ${e.skill_id.slice(0, 12)}... : ${e.c} executions, ${e.success} success`);
    }
  }
} catch {
...
console.log(`Lifecycle: ${archived} archived, ${expired} expired, ${consolidated} consolidated, ${frozen} frozen`);
