...
common edge cases including build failures and health check timeouts.`;
  ...
  const seedRow = db.prepare("SELECT id, text, semantic_type FROM chunks WHERE id = ?").get(seedSkillId) as Record<string, unknown> | undefined;
  ...
  const executions = [
    ...
    { success: true, reward: 0.90, timeMs: 11500 },
  ...
  assert(metrics.successRate >= 0.79 && metrics.successRate <= 0.81, `~80% success rate (got ${(metrics.successRate * 100).toFixed(0)}%)`);
  assert(metrics.avgRewardScore > 0.5, `Avg reward > 0.5 (got ${metrics.avgRewardScore.toFixed(2)})`);
  ...
  if (crystallized >= 1) {
    ...
    assert(false, `Expected at least 1 crystallized skill (got ${crystallized})`, "Check MIN_SUCCESSES/MIN_SUCCESS_RATE thresholds");
  }
...
- Extended rollback window covers slow-burn failures`,
  ...
  try {
    ...
    console.log(`  Score: ${mutResult.score.toFixed(3)} | Promoted: ${mutResult.promoted} | Reason: ${mutResult.reason}`);
  } catch (err) {
  ...
  if (bestSkill) {
    const skillRow = db.prepare("SELECT text, path FROM chunks WHERE id = ?").get(String(bestSkill.id)) as Record<string, unknown> | undefined;
    if (skillRow) {
  ...
  try {
    ...
    assert(ingestResult.ok === true, `Peer skill ingested successfully (action: ${ingestResult.action})`);

    if (ingestResult.ok && ingestResult.crystalId) {
      const imported = db.prepare("SELECT id, lifecycle, semantic_type, origin FROM chunks WHERE id = ?").get(ingestResult.crystalId) as Record<string, unknown> | undefined;
  ...
  if (allSkills.length > 0) {
    const skillWithProvenance = db.prepare(
      "SELECT provenance_chain, provenance_dag, governance_json FROM chunks WHERE id = ?",
  ...
  if (allSkills.length > 0) {
    const marketplaceRow = db.prepare(
  ...
  console.log(`  Economic summary: earnings=$${summary.totalEarningsUsdc.toFixed(4)}, listed=${summary.listedSkillCount}`);

  // ── Phase 9: Consolidation Immunity ───────────────────────────────
  section("Phase 9: Consolidation Immunity (Frozen = Survives Decay)");

  const preConsolSkillCount = (db.prepare(
  ...
  console.log(`  Consolidation: ${consolStats.totalChunks} chunks, ${consolStats.forgottenChunks} forgotten, ${consolStats.mergedChunks} merged`);

  const postConsolSkillCount = (db.prepare(
    "SELECT COUNT(*) as c FROM chunks WHERE semantic_type = 'skill' AND lifecycle = 'frozen'",
  ).get() as { c: number }).c;
  assert(postConsolSkillCount >= preConsolSkillCount, `All frozen skills survived consolidation (${postConsolSkillCount} >= ${preConsolSkillCount})`);
