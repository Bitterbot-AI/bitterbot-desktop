...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".bitterbot-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + bitterbot-purple). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateBitterbotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_BITTERBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_BITTERBOT_BROWSER_COLOR).toUpperCase();
