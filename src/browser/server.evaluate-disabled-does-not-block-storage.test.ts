  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          bitterbot: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.BITTERBOT_GATEWAY_PORT;
    process.env.BITTERBOT_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.BITTERBOT_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.BITTERBOT_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.BITTERBOT_GATEWAY_PORT = prevGatewayPort;
    }
    if (prevGatewayToken === undefined) {
      delete process.env.BITTERBOT_GATEWAY_TOKEN;
    } else {
      process.env.BITTERBOT_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.BITTERBOT_GATEWAY_PASSWORD;
    } else {
      process.env.BITTERBOT_GATEWAY_PASSWORD = prevGatewayPassword;
    }
