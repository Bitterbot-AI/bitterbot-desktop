...
beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "bitterbot",
        profiles: {
          bitterbot: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchBitterbotChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveBitterbotUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopBitterbotChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.BITTERBOT_GATEWAY_PORT;
    process.env.BITTERBOT_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.BITTERBOT_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.BITTERBOT_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.BITTERBOT_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.BITTERBOT_GATEWAY_TOKEN;
    } else {
      process.env.BITTERBOT_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.BITTERBOT_GATEWAY_PASSWORD;
    } else {
      process.env.BITTERBOT_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
