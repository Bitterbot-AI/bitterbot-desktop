...
vi.mock("./chrome.js", () => ({
  ...
  launchBitterbotChrome: vi.fn(async () => {
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const state = makeState("bitterbot");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const bitterbot = ctx.forProfile("bitterbot");

    const opened = await bitterbot.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("bitterbot")?.lastTargetId).toBe("CREATED");
  });
});
