...
vi.mock("./chrome.js", () => ({
  resolveBitterbotUserDataDir: vi.fn(() => "/tmp/bitterbot-test/bitterbot/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveBitterbotUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "bitterbot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveBitterbotUserDataDir).mockReturnValue(userDataDir);
