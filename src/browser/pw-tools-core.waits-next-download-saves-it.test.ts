...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredBitterbotTmpDir: vi.fn(() => "/tmp/bitterbot"),
}));
vi.mock("../infra/tmp-bitterbot-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredBitterbotTmpDir.mockReturnValue("/tmp/bitterbot");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    ...
    tmpDirMocks.resolvePreferredBitterbotTmpDir.mockReturnValue("/tmp/bitterbot-preferred");
    setPwToolsCoreCurrentPage({ on, off });
    ...
    const expectedDownloadsTail = `${path.join("tmp", "bitterbot-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredBitterbotTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "bitterbot-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "bitterbot-preferred", "downloads")}${path.sep}`),
    );
  });
