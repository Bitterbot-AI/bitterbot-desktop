import type { BrowserConfig, BrowserProfileConfig, BitterbotConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_BITTERBOT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "bitterbot" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_BITTERBOT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_BITTERBOT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Bitterbot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for bitterbot/bitterbot CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: BitterbotConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_BITTERBOT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "bitterbot";
