...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { BITTERBOT_STATE_DIR: process.env.BITTERBOT_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.BITTERBOT_STATE_DIR === undefined) {
      delete process.env.BITTERBOT_STATE_DIR;
    } else {
      process.env.BITTERBOT_STATE_DIR = snapshot.BITTERBOT_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "bitterbot-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "bitterbot-memory-flush-"));
  });
