...
import { scheduleGatewaySigusr1Restart, triggerBitterbotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Bitterbot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerBitterbotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Bitterbot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
