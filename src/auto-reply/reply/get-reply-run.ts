...
type RunPreparedReplyParams = {
  ...
  sessionCfg: BitterbotConfig["session"];
...
export async function runPreparedReply(
  ...
  // Inject project context if an active project is specified
  ...
  if (projectId) {
    try {
      ...
      if (projectCtx) {
        ...
        if (parts.length > 0) {
          projectContextPrompt = parts.join("\n\n");
        }
      }
    } catch {
      // Silently skip project context if it fails to load
    }
  }
