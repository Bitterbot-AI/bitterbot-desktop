...
export function isCommandEnabled(cfg: BitterbotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: BitterbotConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as BitterbotConfig),
...
export function getCommandDetection(_cfg?: BitterbotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: BitterbotConfig) {
