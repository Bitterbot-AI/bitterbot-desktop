...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["BITTERBOT_BASE_ENV"]);
    process.env.BITTERBOT_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.BITTERBOT_BASE_ENV ?? "") + "|" + (process.env.BITTERBOT_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { BITTERBOT_TEST_ENV: "ok" },
        },
      );
