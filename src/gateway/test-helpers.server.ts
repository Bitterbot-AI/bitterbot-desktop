...
async function setupGatewayTestHome() {
  ...
  previousConfigPath = process.env.BITTERBOT_CONFIG_PATH;
  previousSkipBrowserControl = process.env.BITTERBOT_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.BITTERBOT_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.BITTERBOT_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.BITTERBOT_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.BITTERBOT_SKIP_CHANNELS;
  previousSkipProviders = process.env.BITTERBOT_SKIP_PROVIDERS;
  previousSkipCron = process.env.BITTERBOT_SKIP_CRON;
  previousMinimalGateway = process.env.BITTERBOT_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.BITTERBOT_STATE_DIR = path.join(tempHome, ".bitterbot");
  delete process.env.BITTERBOT_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.BITTERBOT_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.BITTERBOT_SKIP_GMAIL_WATCHER = "1";
  process.env.BITTERBOT_SKIP_CANVAS_HOST = "1";
  process.env.BITTERBOT_SKIP_CHANNELS = "1";
  process.env.BITTERBOT_SKIP_PROVIDERS = "1";
  process.env.BITTERBOT_SKIP_CRON = "1";
  process.env.BITTERBOT_TEST_MINIMAL_GATEWAY = "1";
  process.env.BITTERBOT_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "bitterbot-test-no-bundled-extensions")
    : "bitterbot-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "bitterbot-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".bitterbot-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.BITTERBOT_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.BITTERBOT_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.BITTERBOT_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.BITTERBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.BITTERBOT_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.BITTERBOT_SKIP_CANVAS_HOST;
    } else {
      process.env.BITTERBOT_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.BITTERBOT_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.BITTERBOT_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.BITTERBOT_SKIP_CHANNELS;
    } else {
      process.env.BITTERBOT_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.BITTERBOT_SKIP_PROVIDERS;
    } else {
      process.env.BITTERBOT_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.BITTERBOT_SKIP_CRON;
    } else {
      process.env.BITTERBOT_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.BITTERBOT_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.BITTERBOT_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["BITTERBOT_GATEWAY_TOKEN"]);
  const prev = process.env.BITTERBOT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.BITTERBOT_GATEWAY_TOKEN = fallbackToken;
  }
