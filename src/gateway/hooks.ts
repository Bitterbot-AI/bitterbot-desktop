...
export function resolveHooksConfig(cfg: BitterbotConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: BitterbotConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-bitterbot-token"] === "string"
      ? req.headers["x-bitterbot-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
