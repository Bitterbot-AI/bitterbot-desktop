...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-Bitterbot-Token header (query parameters are not allowed).",
      );
      return true;
    }
...
export function createGatewayHttpServer(opts: {
  ...
  handleA2aRequest?: A2aRequestHandler;
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      if (requestPath === "/management" && req.method === "GET") {
        const token = getBearerToken(req);
        const authResult = await authorizeGatewayConnect({
          auth: resolvedAuth,
          connectAuth: token ? { token, password: token } : null,
      ...
      if (requestPath === "/wallet/fund" && req.method === "GET") {
        const token = getBearerToken(req);
        const authResult = await authorizeGatewayConnect({
          auth: resolvedAuth,
          connectAuth: token ? { token, password: token } : null,
        ...
        res.setHeader("Content-Type", "text/html; charset=utf-8");
        const gatewayToken = resolvedAuth?.token ?? "";
        res.end(renderWalletFundingPage(wsUrl, gatewayToken));
        return;
      }
      if (handleA2aRequest) {
        if (
