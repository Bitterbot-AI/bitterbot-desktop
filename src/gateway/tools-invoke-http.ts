...
export async function handleToolsInvokeHttpRequest(
      ...
      sendJson(res, 400, {
        ...
        error: {
          ...
          message:
            ...
            "Memory tools are built-in. Ensure memory is enabled in your config.",
        },
      });
      return true;
    }
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-bitterbot-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-bitterbot-account-id")?.trim() || undefined;
  ...
  const allTools = createBitterbotTools({
