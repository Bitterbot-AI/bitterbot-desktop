...
beforeAll(async () => {
  previousToken = process.env.BITTERBOT_GATEWAY_TOKEN;
  process.env.BITTERBOT_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.BITTERBOT_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "bitterbot-ios", platform: "ios" },
  { clientId: "bitterbot-android", platform: "android" },
