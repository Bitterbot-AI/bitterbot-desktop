...
export function createAgentEventHandler({
  ...
  return (evt: AgentEventPayload) => {
    ...
    const isArtifactEvent = evt.stream === "artifact";
    if (isArtifactEvent) {
      // Forward artifact events as a dedicated "artifact" broadcast event
      broadcast("artifact", {
        runId: evt.runId,
        seq: evt.seq,
        ts: evt.ts,
    ...
    if (isToolEvent && evt.data?.phase === "result" && !evt.data?.isError) {
      const toolName = typeof evt.data.name === "string" ? evt.data.name.toLowerCase() : "";
      const isFileWrite =
