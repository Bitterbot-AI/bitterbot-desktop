...
const CLI_LIVE = isTruthyEnvValue(process.env.BITTERBOT_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.BITTERBOT_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.BITTERBOT_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("BITTERBOT_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    const previous = {
      configPath: process.env.BITTERBOT_CONFIG_PATH,
      token: process.env.BITTERBOT_GATEWAY_TOKEN,
      skipChannels: process.env.BITTERBOT_SKIP_CHANNELS,
      skipGmail: process.env.BITTERBOT_SKIP_GMAIL_WATCHER,
      skipCron: process.env.BITTERBOT_SKIP_CRON,
      skipCanvas: process.env.BITTERBOT_SKIP_CANVAS_HOST,
    ...
    process.env.BITTERBOT_GATEWAY_TOKEN = token;

    const rawModel = process.env.BITTERBOT_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `BITTERBOT_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.BITTERBOT_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
        `BITTERBOT_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
      );
    }
    const baseCliArgs =
      parseJsonStringArray(
        "BITTERBOT_LIVE_CLI_BACKEND_ARGS",
        process.env.BITTERBOT_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`BITTERBOT_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "BITTERBOT_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.BITTERBOT_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.BITTERBOT_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.BITTERBOT_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "BITTERBOT_LIVE_CLI_BACKEND_IMAGE_MODE requires BITTERBOT_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-live-cli-"));
    const disableMcpConfig = process.env.BITTERBOT_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
    ...
    const tempConfigPath = path.join(tempDir, "bitterbot.json");
    await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
    process.env.BITTERBOT_CONFIG_PATH = tempConfigPath;
