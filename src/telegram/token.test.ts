...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "bitterbot-telegram-token-"));
}

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.BITTERBOT_STATE_DIR;
  const dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "bitterbot-telegram-"));
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.BITTERBOT_STATE_DIR = previous;
    }
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as BitterbotConfig;
    const res = resolveTelegramToken(cfg);
