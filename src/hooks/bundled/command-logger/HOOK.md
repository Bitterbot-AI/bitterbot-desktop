...
homepage: https://docs.bitterbot.ai/automation/hooks#command-logger
metadata:
  {
    "bitterbot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.bitterbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.bitterbot/logs/commands.log ~/.bitterbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/bitterbot`:
   ```
   /home/username/.bitterbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
