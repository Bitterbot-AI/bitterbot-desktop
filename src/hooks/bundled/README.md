...
This directory contains hooks that ship with Bitterbot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Injects extra bootstrap files (for example monorepo `TOOLS.md`/`PROTOCOLS.md`) during prompt assembly.
...
Hooks can be configured in `~/.bitterbot/bitterbot.json`:
...
2. Restart gateway: `pkill -9 -f 'bitterbot.*gateway' && pnpm bitterbot gateway`
3. Enable the hook: `bitterbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.bitterbot.ai/automation/hooks
