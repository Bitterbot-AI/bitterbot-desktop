...
`;
    ...
    expect(parsed.bitterbot.emoji).toBe("💾");
    expect(parsed.bitterbot.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.bitterbot.emoji).toBe("📝");
    expect(parsed.bitterbot.events).toEqual(["command"]);
    expect(parsed.bitterbot.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.bitterbot.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"bitterbot": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"bitterbot": {"events": ["test"]}}');
  });
...
describe("resolveBitterbotMetadata", () => {
  it("extracts bitterbot metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveBitterbotMetadata(frontmatter);
  ...
  it("returns undefined when bitterbot key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        bitterbot: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Bitterbot" },
            { id: "npm", kind: "npm", package: "@bitterbot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@bitterbot/hook");
  });
...
`;
    ...
    const bitterbot = resolveBitterbotMetadata(frontmatter);
    expect(bitterbot).toBeDefined();
    expect(bitterbot?.emoji).toBe("💾");
    expect(bitterbot?.events).toEqual(["command:new"]);
    expect(bitterbot?.requires?.config).toEqual(["workspace.dir"]);
    expect(bitterbot?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const bitterbot = resolveBitterbotMetadata(frontmatter);
    expect(bitterbot?.emoji).toBe("disk");
    expect(bitterbot?.events).toEqual(["command:new"]);
  });
});
