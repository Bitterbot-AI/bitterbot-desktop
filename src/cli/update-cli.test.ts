...
vi.mock("../infra/bitterbot-root.js", () => ({
  resolveBitterbotPackageRoot: vi.fn(),
}));
...
const { resolveBitterbotPackageRoot } = await import("../infra/bitterbot-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-update-tests-"));
  });
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("bitterbot-update");
    setTty(false);
    readPackageVersion.mockResolvedValue("2.0.0");

    vi.mocked(resolveBitterbotPackageRoot).mockResolvedValue(tempDir);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveBitterbotPackageRoot).mockReset();
    ...
    vi.mocked(resolveBitterbotPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("bitterbot");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Bitterbot update status");
  });
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("bitterbot-update-wizard");
    const envSnapshot = captureEnv(["BITTERBOT_GIT_DIR"]);
    try {
      setTty(true);
      process.env.BITTERBOT_GIT_DIR = tempDir;
