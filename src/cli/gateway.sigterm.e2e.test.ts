...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "bitterbot-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const bootstrapPath = path.join(stateDir, "bitterbot-entry-bootstrap.mjs");
