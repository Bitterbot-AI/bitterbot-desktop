...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.bitterbot.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Bitterbot hooks")
    ...
    .option("--hook-url <url>", "Bitterbot hook URL")
    .option("--hook-token <token>", "Bitterbot hook token")
