...
${formatHelpExamples([
  ['bitterbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['bitterbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['bitterbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.bitterbot.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.bitterbot.ai/cli/agents")}\n`,
    );
  ...
  agents
    ...
    .description("Update an agent display identity (name/theme/emoji/avatar)")
