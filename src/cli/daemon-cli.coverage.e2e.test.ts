...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    BITTERBOT_STATE_DIR: process.env.BITTERBOT_STATE_DIR,
    BITTERBOT_CONFIG_PATH: process.env.BITTERBOT_CONFIG_PATH,
    BITTERBOT_GATEWAY_PORT: process.env.BITTERBOT_GATEWAY_PORT,
    BITTERBOT_PROFILE: process.env.BITTERBOT_PROFILE,
  };

  beforeEach(() => {
    process.env.BITTERBOT_STATE_DIR = "/tmp/bitterbot-cli-state";
    process.env.BITTERBOT_CONFIG_PATH = "/tmp/bitterbot-cli-state/bitterbot.json";
    delete process.env.BITTERBOT_GATEWAY_PORT;
    delete process.env.BITTERBOT_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.BITTERBOT_STATE_DIR !== undefined) {
      process.env.BITTERBOT_STATE_DIR = originalEnv.BITTERBOT_STATE_DIR;
    } else {
      delete process.env.BITTERBOT_STATE_DIR;
    }

    if (originalEnv.BITTERBOT_CONFIG_PATH !== undefined) {
      process.env.BITTERBOT_CONFIG_PATH = originalEnv.BITTERBOT_CONFIG_PATH;
    } else {
      delete process.env.BITTERBOT_CONFIG_PATH;
    }

    if (originalEnv.BITTERBOT_GATEWAY_PORT !== undefined) {
      process.env.BITTERBOT_GATEWAY_PORT = originalEnv.BITTERBOT_GATEWAY_PORT;
    } else {
      delete process.env.BITTERBOT_GATEWAY_PORT;
    }

    if (originalEnv.BITTERBOT_PROFILE !== undefined) {
      process.env.BITTERBOT_PROFILE = originalEnv.BITTERBOT_PROFILE;
    } else {
      delete process.env.BITTERBOT_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        BITTERBOT_PROFILE: "dev",
        BITTERBOT_STATE_DIR: "/tmp/bitterbot-daemon-state",
        BITTERBOT_CONFIG_PATH: "/tmp/bitterbot-daemon-state/bitterbot.json",
        BITTERBOT_GATEWAY_PORT: "19001",
      },
