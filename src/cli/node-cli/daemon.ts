...
function resolveNodeDefaults(
  ...
  const port = portOverride ?? config?.gateway?.port ?? 19001;
  return { host, port };
}
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("bitterbot node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    BITTERBOT_LOG_PREFIX: baseEnv.BITTERBOT_LOG_PREFIX ?? "node",
