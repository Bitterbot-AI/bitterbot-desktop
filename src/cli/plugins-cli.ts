...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: BitterbotConfig; warnings: string[] } {
...
function enablePluginInConfig(config: BitterbotConfig, pluginId: string): BitterbotConfig {
  return {
    ...config,
    plugins: {
      ...config.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage Bitterbot plugins/extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.bitterbot.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(async (id: string, opts: PluginUninstallOptions) => {
      ...
      if (cfg.plugins?.slots?.memory === pluginId) {
        preview.push("memory slot (will be cleared)");
      }
    ...
    .action(async (raw: string, opts: { link?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: BitterbotConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.bitterbot.ai/plugin");
      lines.push("");
