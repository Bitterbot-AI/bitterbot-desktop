...
export function unauthorizedHintForMessage(message: string): string | null {
  ...
  if (
    ...
    return [
      "Node bridge rejected the client.",
      "Sign the Bitterbot CLI (TeamID Y5PE65HELJ) or launch the host with",
      "BITTERBOT_ALLOW_UNSIGNED_SOCKET_CLIENTS=1 for local dev.",
    ].join(" ");
  }
  return null;
}
