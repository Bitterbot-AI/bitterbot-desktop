...
function listAgents(cfg: BitterbotConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: BitterbotConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: BitterbotConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<BitterbotConfig, EvaluatedBindingsCache>();
