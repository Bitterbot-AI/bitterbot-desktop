...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.BITTERBOT_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.BITTERBOT_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "bitterbot");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.BITTERBOT_STATE_DIR = stateDir;
    process.env.BITTERBOT_CONFIG_PATH = path.join(stateDir, "bitterbot.json");
