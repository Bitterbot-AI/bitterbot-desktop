...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Bitterbot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("bitterbot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("bitterbot security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.bitterbot.ai/faq");
  runtime.log("Troubleshooting: https://docs.bitterbot.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("bitterbot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("bitterbot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("bitterbot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("bitterbot gateway probe")}`);
  }
}
