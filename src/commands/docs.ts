...
const SEARCH_TOOL = "https://docs.bitterbot.ai/mcp.SearchBitterbot";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.bitterbot.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('bitterbot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.bitterbot.ai/");
      runtime.log(`Search: ${formatCliCommand('bitterbot docs "your query"')}`);
    }
    return;
  }
