...
const requireAgentDir = () => {
  const agentDir = process.env.BITTERBOT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("BITTERBOT_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under BITTERBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-oauth-"));
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to BITTERBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-minimax-"));
    process.env.BITTERBOT_STATE_DIR = tempStateDir;
    process.env.BITTERBOT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.BITTERBOT_AGENT_DIR;
