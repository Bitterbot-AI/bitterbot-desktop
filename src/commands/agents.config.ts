...
export function listAgentEntries(cfg: BitterbotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: BitterbotConfig, agentId: string) {
...
function resolveAgentModel(cfg: BitterbotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: BitterbotConfig): AgentSummary[] {
