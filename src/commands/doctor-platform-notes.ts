...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".bitterbot", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: BitterbotConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [
    ["BITTERBOT_GATEWAY_TOKEN", await getenv("BITTERBOT_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["BITTERBOT_GATEWAY_PASSWORD", await getenv("BITTERBOT_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "BITTERBOT_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
