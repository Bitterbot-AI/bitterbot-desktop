...
beforeAll(() => {
  envSnapshot = captureEnv(["BITTERBOT_PROFILE"]);
  process.env.BITTERBOT_PROFILE = "isolated";
});
...
vi.mock("../infra/bitterbot-root.js", () => ({
  resolveBitterbotPackageRoot: vi.fn().mockResolvedValue("/tmp/bitterbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/bitterbot/pnpm-lock.yaml",
      markerPath: "/tmp/bitterbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("Bitterbot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("bitterbot status --all") ||
          l.includes("bitterbot --profile isolated status --all") ||
          l.includes("bitterbot status --all") ||
          l.includes("bitterbot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.BITTERBOT_GATEWAY_TOKEN = "abcd1234";
    try {
