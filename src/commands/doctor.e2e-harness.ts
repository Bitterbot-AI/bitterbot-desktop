...
export const resolveBitterbotPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
export const resolveGatewayProgramArguments = vi.fn().mockResolvedValue({
  programArguments: ["node", "cli", "gateway", "--port", "19001"],
}) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadBitterbotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/bitterbot.json",
    createConfigIO,
...
vi.mock("../infra/bitterbot-root.js", () => ({
  resolveBitterbotPackageRoot,
}));
...
beforeEach(() => {
  ...
  resolveBitterbotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.BITTERBOT_STATE_DIR;
  originalUpdateInProgress = process.env.BITTERBOT_UPDATE_IN_PROGRESS;
  process.env.BITTERBOT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "bitterbot-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.BITTERBOT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.BITTERBOT_UPDATE_IN_PROGRESS;
  } else {
    process.env.BITTERBOT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
