...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveBitterbotAgentDir: vi.fn().mockReturnValue("/tmp/bitterbot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/bitterbot-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/bitterbot-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveBitterbotAgentDir: mocks.resolveBitterbotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveBitterbotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/bitterbot-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/bitterbot-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/bitterbot-agent-custom");
