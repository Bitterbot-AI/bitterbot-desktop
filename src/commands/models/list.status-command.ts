...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveBitterbotAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("bitterbot models auth setup-token")}\` or \`${formatCliCommand("bitterbot configure")}\`.`
          : `Run \`${formatCliCommand("bitterbot configure")}\` or set an API key env var.`;
