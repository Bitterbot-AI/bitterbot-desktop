...
export async function updateConfig(
  mutator: (cfg: BitterbotConfig) => BitterbotConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: BitterbotConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: BitterbotConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("bitterbot agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
