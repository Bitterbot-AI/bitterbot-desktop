...
function formatSummary(summary: AgentSummary) {
  ...
  const identitySource = summary.identitySource === "config" ? "config" : null;
...
export async function agentsListCommand(
  ...
  lines.push(
    `Channel status reflects local config/creds. For live health: ${formatCliCommand("bitterbot channels status --probe")}.`,
  );
