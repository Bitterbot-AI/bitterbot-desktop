...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: BitterbotConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bitterbot-security-audit-"));
  });
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.BITTERBOT_GATEWAY_TOKEN;
    const prevPassword = process.env.BITTERBOT_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevPassword === undefined) {
        ...
        process.env.BITTERBOT_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: BitterbotConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    ...
    process.env.BITTERBOT_STATE_DIR = tmp;
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: BitterbotConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.BITTERBOT_GATEWAY_TOKEN;
    process.env.BITTERBOT_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: BitterbotConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.bitterbot",
      configPath: "/Users/test/Dropbox/.bitterbot/bitterbot.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: BitterbotConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "bitterbot.json"),
      });
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        bitterbot: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        bitterbot: { extensions: ["../outside.js"] },
      }),
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.BITTERBOT_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.BITTERBOT_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.BITTERBOT_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.BITTERBOT_GATEWAY_PASSWORD;
      } else {
        process.env.BITTERBOT_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.BITTERBOT_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: BitterbotConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.BITTERBOT_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: BitterbotConfig = {
        gateway: {
          mode: "remote",
