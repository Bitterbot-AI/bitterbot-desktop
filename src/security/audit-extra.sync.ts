...
function summarizeGroupPolicy(cfg: BitterbotConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: BitterbotConfig): boolean {
...
function collectModels(cfg: BitterbotConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: BitterbotConfig): Set<string> {
  const baseCfg: BitterbotConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: BitterbotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: BitterbotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: BitterbotConfig): boolean {
...
function isBrowserEnabled(cfg: BitterbotConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: BitterbotConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: BitterbotConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep BITTERBOT_STATE_DIR on a local-only volume and re-run "${formatCliCommand("bitterbot security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: BitterbotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer BITTERBOT_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const bitterbotGatewayToken =
    typeof env.BITTERBOT_GATEWAY_TOKEN === "string" && env.BITTERBOT_GATEWAY_TOKEN.trim()
      ? env.BITTERBOT_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-bitterbot-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: BitterbotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: BitterbotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: BitterbotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
