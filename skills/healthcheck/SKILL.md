---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Bitterbot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Bitterbot cron scheduling for periodic checks, or version status checks on a machine running Bitterbot (laptop, workstation, Pi, VPS).
---

# Bitterbot Host Hardening

## Overview

Assess and harden the host running Bitterbot, then align it to a user-defined risk tolerance without breaking access. Use Bitterbot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Bitterbot changes the host firewall, SSH, or OS updates; it does not.
...
5. Bitterbot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Bitterbot can access sensitive data.
...
### 2) Run Bitterbot security audits (read-only)

As part of the default read-only checks, run `bitterbot security audit --deep`. Only offer alternatives if the user requests them:

1. `bitterbot security audit` (faster, non-probing)
2. `bitterbot security audit --json` (structured output)

Offer to apply Bitterbot safe defaults (numbered):

1. `bitterbot security audit --fix`

Be explicit that `--fix` only tightens Bitterbot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Bitterbot version/update status (read-only)

As part of the default read-only checks, run `bitterbot update status`.
...
- Credential hygiene notes (location of Bitterbot creds, prefer disk encryption)
...
- Bitterbot security audit (re-run)
...
After Bitterbot install or first hardening pass, run at least one baseline audit and version check:

- `bitterbot security audit`
- `bitterbot security audit --deep`
- `bitterbot update status`

Ongoing monitoring is recommended. Use the Bitterbot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `bitterbot cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `bitterbot update status`
...
Before creating, `bitterbot cron list` and match on exact `name`. If found, `bitterbot cron edit <id> ...`.
If not found, `bitterbot cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `bitterbot update status` (preferred for source checkouts and channels)
2. `npm view bitterbot version` (published npm version)

## Bitterbot command accuracy

Use only supported commands and flags:

- `bitterbot security audit [--deep] [--fix] [--json]`
- `bitterbot status` / `bitterbot status --deep`
- `bitterbot health --json`
- `bitterbot update status`
- `bitterbot cron add|list|runs|run`

Do not invent CLI flags or imply Bitterbot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Bitterbot compaction:
