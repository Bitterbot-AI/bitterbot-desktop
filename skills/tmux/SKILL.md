...
metadata:
  { "bitterbot": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
- Use `BITTERBOT_TMUX_SOCKET_DIR`.
- Default socket path: `"$BITTERBOT_TMUX_SOCKET_DIR/bitterbot.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `BITTERBOT_TMUX_SOCKET_DIR`).
