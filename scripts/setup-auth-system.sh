#!/bin/bash
# Setup Bitterbot Auth Management System
...
echo "=== Bitterbot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'bitterbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Bitterbot message: Send warning via Bitterbot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/bitterbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/bitterbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/bitterbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now bitterbot-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/BitterbotAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/BitterbotAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Bitterbot*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (BitterbotAuth) shows status and opens auth URL if needed."
echo "The full widget (BitterbotAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status bitterbot-auth-monitor.timer"
echo ""
echo "Quick commands:"
