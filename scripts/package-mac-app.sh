...
# Build and bundle Bitterbot into a minimal .app we can open.
# Outputs to dist/Bitterbot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Bitterbot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Bitterbot"
BUNDLE_ID="${BUNDLE_ID:-ai.bitterbot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/bitterbot/bitterbot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Bitterbot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :BitterbotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :BitterbotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Bitterbot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Bitterbot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Bitterbot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Bitterbot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Bitterbot/Resources/Bitterbot.icns" "$APP_ROOT/Contents/Resources/Bitterbot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Bitterbot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying BitterbotKit resources"
BITTERBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/BitterbotKit_BitterbotKit.bundle"
if [ -d "$BITTERBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/BitterbotKit_BitterbotKit.bundle"
  cp -R "$BITTERBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/BitterbotKit_BitterbotKit.bundle"
else
  echo "WARN: BitterbotKit resource bundle not found at $BITTERBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Bitterbot"
killall -q Bitterbot 2>/dev/null || true
