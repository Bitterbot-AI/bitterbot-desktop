...
ZIP=${1:?"Usage: $0 Bitterbot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/bitterbot/bitterbot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Bitterbot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/bitterbot/bitterbot/releases/download/v${VERSION}/"}
