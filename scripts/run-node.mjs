...
const shouldBuild = (deps) => {
  if (deps.env.BITTERBOT_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.BITTERBOT_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[bitterbot] ${message}\n`);
};

const runBitterbot = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["bitterbot.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
