...
#
# Suggested cron: */30 * * * * /home/admin/bitterbot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Bitterbot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., bitterbot-alerts)
...
STATE_FILE="$HOME/.bitterbot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Bitterbot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use bitterbot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Bitterbot to $NOTIFY_PHONE..."
            bitterbot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Bitterbot is down. Run: ssh l36 '~/bitterbot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
