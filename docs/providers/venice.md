---
summary: "Use Venice AI privacy-focused models in Bitterbot"
read_when:
  - You want privacy-focused inference in Bitterbot
...
```bash
bitterbot onboard --auth-choice venice-api-key
```
...
```bash
bitterbot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Bitterbot shows all available Venice models. Pick based on your needs:
...
```bash
bitterbot models set venice/claude-opus-45
bitterbot models set venice/llama-3.3-70b
```

List all available models:

```bash
bitterbot models list | grep venice
```

## Configure via `bitterbot configure`

1. Run `bitterbot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Bitterbot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
bitterbot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
bitterbot chat --model venice/claude-opus-45

# Use uncensored model
bitterbot chat --model venice/venice-uncensored

# Use vision model with image
bitterbot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
bitterbot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `bitterbot models list` to see currently available models. Some models may be temporarily offline.
