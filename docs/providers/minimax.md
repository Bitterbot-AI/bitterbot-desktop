---
summary: "Use MiniMax M2.1 in Bitterbot"
read_when:
  - You want MiniMax models in Bitterbot
...
```bash
bitterbot plugins enable minimax-portal-auth  # skip if already loaded.
bitterbot gateway restart  # restart if gateway is already running
bitterbot onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/Bitterbot-AI/bitterbot-desktop/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `bitterbot.json`:
...
- Use `bitterbot models list` and `bitterbot models set minimax/MiniMax-M2.1` to switch.
...
- Running `bitterbot configure` and selecting **MiniMax M2.1**, or
