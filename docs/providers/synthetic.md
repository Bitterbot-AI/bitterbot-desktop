---
summary: "Use Synthetic's Anthropic-compatible API in Bitterbot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Bitterbot registers it as the
...
```bash
bitterbot onboard --auth-choice synthetic-api-key
```
...
Note: Bitterbot's Anthropic client appends `/v1` to the base URL, so use
