...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `bitterbot doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
...
```
    ...
    - or read `chat.id` from `bitterbot logs --follow`
...
- DM messages can carry `message_thread_id`; Bitterbot routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Bitterbot can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    For text-only replies, Bitterbot keeps the same preview message and performs a final edit in place (no second message).

    For complex replies (for example media payloads), Bitterbot falls back to normal final delivery and then cleans up the preview message.

    `streamMode` is separate from block streaming. When block streaming is explicitly enabled for Telegram, Bitterbot skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Bitterbot retries as plain text.
...
```
    ...
    2. paste code in your node's gateway settings
    3. `/pair approve` approves latest pending request

    More details: [Pairing](/channels/pairing#pair-via-telegram).
...
```
    ...
    - `~/.bitterbot/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Bitterbot enqueues system events like:
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while Bitterbot is processing an inbound message.
...
```bash
bitterbot message send --channel telegram --target 123456789 --message "hi"
bitterbot message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `bitterbot channels status` warns when config expects unmentioned group messages.
    - `bitterbot channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `bitterbot logs --follow` for skip reasons
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `bitterbot doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `bitterbot doctor --fix` can resolve legacy `@username` entries to IDs.
