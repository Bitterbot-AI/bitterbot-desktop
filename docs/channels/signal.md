...
- Bitterbot installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "Bitterbot"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure Bitterbot and restart the gateway.
5. Send a first DM and approve pairing (`bitterbot pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "Bitterbot"` then scan the QR in Signal.
...
Multi-account support: use `channels.signal.accounts` with per-account config and optional `name`. See [`gateway/configuration`](/gateway/configuration) for the shared pattern.
...
4. Configure Bitterbot, restart gateway, verify channel:

```bash
# If you run the gateway as a user systemd service:
systemctl --user restart bitterbot-gateway
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `bitterbot pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point Bitterbot at it:
...
This skips auto-spawn and the startup wait inside Bitterbot. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `bitterbot pairing list signal`
  - `bitterbot pairing approve signal <CODE>`
...
- **Typing indicators**: Bitterbot sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Bitterbot forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `bitterbot doctor --fix`.
...
bitterbot pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/bitterbot.bitterbot-$(date +%Y-%m-%d).log" | tail -20
```
