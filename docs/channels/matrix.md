...
Matrix is an open, decentralized messaging protocol. Bitterbot connects as a Matrix **user**
...
```bash
bitterbot plugins install @bitterbot/matrix
```

Local checkout (when running from a git repo):

```bash
bitterbot plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `bitterbot plugins install @bitterbot/matrix`
   - From a local checkout: `bitterbot plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Bitterbot calls the same
     login endpoint, stores the access token in `~/.bitterbot/credentials/matrix/credentials.json`,
...
- On first connection, Bitterbot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Bitterbot logs a warning.
...
`~/.bitterbot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `bitterbot pairing list matrix`
  - `bitterbot pairing approve matrix <CODE>`
...
- On startup, Bitterbot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
