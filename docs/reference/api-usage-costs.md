...
Bitterbot features that can generate provider usage or paid API calls.
...
Bitterbot can pick up credentials from:
...
- `bitterbot status --usage`
- `bitterbot models status --json`
...
`bitterbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
