...
read_when:
  - Running Bitterbot behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Bitterbot
...
- You run Bitterbot behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Bitterbot checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Bitterbot extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://bitterbot.example.com
    to: http://bitterbot-gateway:19001
    policy:
      - allow:
          or:
            - email:
...
```
bitterbot.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy bitterbot:19001 {
...
location / {
    ...
    proxy_pass http://bitterbot:19001;
...
`bitterbot security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Bitterbot config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `bitterbot security audit` and review findings
