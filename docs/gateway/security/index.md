...
- Tighten local perms (`~/.bitterbot` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

Bitterbot is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Bitterbot also attempts a best-effort live Gateway probe.
...
`bitterbot security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${BITTERBOT_GATEWAY_PASSWORD}
```
...
Bitterbot stores session transcripts on disk under `~/.bitterbot/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.bitterbot` (see the audit section below). If you need
...
- If you don’t want remote execution, set security to **deny** and remove node pairing for that node.

## Dynamic skills (watcher / remote nodes)

Bitterbot can refresh the skills list mid-session:
...
- If you install plugins from npm (`bitterbot plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.bitterbot/extensions/<pluginId>/` (or `$BITTERBOT_STATE_DIR/extensions/<pluginId>/`).
  - Bitterbot uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
bitterbot pairing approve <channel> <code>
```
...
By default, Bitterbot routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Bitterbot has two separate “who can trigger me?” layers:
...
- Note: sandboxing is opt-in. If sandbox mode is off, exec runs on the gateway host even though tools.exec.host defaults to sandbox, and host exec does not require approvals unless you set host=gateway and configure exec approvals. An explicit `tools.exec.security: "deny"` is enforced on that path.
...
- “Paste the full contents of ~/.bitterbot or your logs.”
...
4. **Re-run audit**
   - `bitterbot security audit --deep` and confirm the report is clean.
...
- `~/.bitterbot/bitterbot.json`: `600` (user read/write only)
- `~/.bitterbot`: `700` (user only)

`bitterbot doctor` can warn and offer to tighten these permissions.
...
- Default: `19001`
- Config/flags/env: `gateway.port`, `--port`, `BITTERBOT_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__bitterbot__/canvas/` and `/__bitterbot__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
Doctor can generate one for you: `bitterbot doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `BITTERBOT_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `BITTERBOT_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Bitterbot
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Bitterbot verifies the identity by resolving the
...
- Bitterbot will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.bitterbot/` (or `$BITTERBOT_STATE_DIR/`) may contain secrets or private data:

- `bitterbot.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `bitterbot status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@bitterbot", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.bitterbot/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `bitterbot` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Bitterbot clients can connect.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.bitterbot/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.bitterbot/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.bitterbot/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "none",
        },
        // Session tools can reveal sensitive data from transcripts. By default Bitterbot limits these tools
...
1. **Stop it:** terminate your `bitterbot gateway` process (or stop the service).
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `BITTERBOT_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/bitterbot.bitterbot-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.bitterbot/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Bitterbot version
...
Found a vulnerability in Bitterbot? Please report responsibly:

1. Email: [security@bitterbot.ai](mailto:security@bitterbot.ai)
2. Don't post publicly until fixed
3. We'll credit you (unless you prefer anonymity)

---

_"Security is a process, not a product."_ — Someone wise, probably
