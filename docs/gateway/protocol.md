...
Bitterbot. All clients (CLI, web UI, headless nodes) connect over WebSocket
and declare their **role** + **scope** at
handshake time.
...
- If `BITTERBOT_GATEWAY_TOKEN` (or `--token`) is set, `connect.params.auth.token`
