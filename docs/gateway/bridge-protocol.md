...
read_when:
  - Building or debugging node clients (headless node mode)
...
**Note:** Current Bitterbot builds no longer ship the TCP bridge listener; this document is kept for historical reference.
...
- **Pairing + node identity**: node admission is owned by the gateway and tied
  to a per-node token.
- **Discovery UX**: nodes can discover gateways via DNS-SD, or connect
...
`bridgeTlsSha256` as a non-secret hint. Note that DNS-SD TXT records are
...
- Use manual host/port or wide-area DNS‑SD for cross-network setups.
