...
read_when:
  - You need detailed behavior for bitterbot onboard
...
This page is the full reference for `bitterbot onboard`.
...
- Channels and providers (Telegram, WhatsApp, Discord, Slack, Signal)
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.bitterbot/bitterbot.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `bitterbot doctor` before continuing.
  ...
  <Step title="Channels (Advanced flow)">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `bitterbot pairing approve <channel> <code>` or use allowlists.
  </Step>
...
If Control UI assets are missing, the wizard attempts to build them; fallback is `pnpm build` (auto-installs UI deps).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.bitterbot/.env` so launchd can read it.
...
- OAuth credentials: `~/.bitterbot/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.bitterbot/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.bitterbot/credentials/oauth.json` (or `$BITTERBOT_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`bitterbot onboard`](/cli/onboard)
