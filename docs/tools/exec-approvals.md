...
- **gateway host** → `bitterbot` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.bitterbot/exec-approvals.sock",
...
editing. Patterns are **case-insensitive glob matches**.
Patterns should resolve to **binary paths** (basename-only entries are ignored).
Legacy `agents.default` entries are migrated to `agents.main` on load.

Examples:

- `~/Projects/**/bin/bitterbot`
- `~/.local/bin/*`
- `/opt/homebrew/bin/rg`
...
`~/.bitterbot/exec-approvals.json` directly.

CLI: `bitterbot approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
Gateway -> Node Service (WS)
                 ...
                 v
             UI Client (approvals + system.run)
```
