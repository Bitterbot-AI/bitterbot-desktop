---
summary: "Chrome extension: let Bitterbot drive your existing Chrome tab"
read_when:
...
The Bitterbot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate.bitterbot-managed Chrome profile.
...
Bitterbot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Bitterbot release (npm package) as static files. There is no separate “build” step.

After upgrading Bitterbot:

- Re-run `bitterbot browser extension install` to refresh the installed files under your Bitterbot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Bitterbot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Bitterbot to control.
...
- `ON`: attached; Bitterbot can drive that tab.
...
Debugging: `bitterbot sandbox explain`
...
`bitterbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `bitterbot browser extension install` first to copy the extension to a stable location under your Bitterbot state directory.
...
- **This is not isolated** like the dedicated.bitterbot-managed profile.
