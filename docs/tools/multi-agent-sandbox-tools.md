...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `bitterbot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `bitterbot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${BITTERBOT_STATE_DIR:-$HOME/.bitterbot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
