---
summary: "Agent tool surface for Bitterbot (browser, canvas, nodes, message, cron) replacing legacy .bitterbot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing .bitterbot-*` skills
title: "Tools"
---
...
You can globally allow/deny tools via `tools.allow` / `tools.deny` in `bitterbot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Bitterbot logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:bitterbot`: all built-in Bitterbot tools (excludes provider plugins)
...
- `host=node` can target a headless node host (`bitterbot node run`).
...
Control the dedicated Bitterbot-managed browser.
...
- If no `node` is provided, the tool picks a default (single connected node).
- A2UI is v0.8 only (no `createSurface`); the CLI rejects v0.9 JSONL with line errors.
- Quick smoke: `bitterbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `bitterbot gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Bitterbot clamps `tools.sessions.visibility` to `tree`.
