...
```bash
bitterbot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `bitterbot tui --deliver`
...
- Check the Gateway logs: `bitterbot logs --follow`.
- Confirm the agent can run: `bitterbot status` and `bitterbot models status`.
...
- No agents in picker: check `bitterbot agents list` and your routing config.
