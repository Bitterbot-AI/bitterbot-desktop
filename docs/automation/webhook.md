...
read_when:
  ...
  - Wiring external systems into Bitterbot
title: "Webhooks"
---
...
- `channel` optional (string): The messaging channel for delivery. One of: `last`, `whatsapp`, `telegram`, `discord`, `slack`, `signal`. Defaults to `last`.
- `to` optional (string): The recipient identifier for the channel (e.g., phone number for WhatsApp/Signal, chat ID for Telegram, channel ID for Discord/Slack). Defaults to the last recipient in the main session.
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Bitterbot config directory (typically `~/.bitterbot/hooks/transforms`).
...
- `bitterbot webhooks gmail setup` writes `hooks.gmail` config for `bitterbot webhooks gmail run`.
...
```bash
curl -X POST http://127.0.0.1:19001/hooks/wake \
...
```bash
curl -X POST http://127.0.0.1:19001/hooks/agent \
  -H 'x-bitterbot-token: SECRET' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST http://127.0.0.1:19001/hooks/gmail \
  -H 'Authorization: Bearer SECRET' \
  -H 'Content-Type: application/json' \
