...
- Jobs persist under `~/.bitterbot/cron/` so restarts don’t lose schedules.
...
bitterbot cron list
bitterbot cron run <job-id>
bitterbot cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.bitterbot/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `bitterbot cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Bitterbot defaults to `announce`.
...
- Job store: `~/.bitterbot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.bitterbot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.bitterbot/cron/jobs.json",
...
bitterbot cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
bitterbot cron edit <jobId> --agent ops
bitterbot cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
bitterbot cron run <jobId>
bitterbot cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
bitterbot cron edit <jobId> \
...
```bash
bitterbot cron runs --id <jobId> --limit 50
```

Immediate system event without creating a job:

```bash
bitterbot system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`bitterbot system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `BITTERBOT_SKIP_CRON`.
