...
SOUL.md has been fully removed from the system. It is no longer created, loaded, or referenced:

1. **GENOME.md is the sole identity blueprint.** The system prompt loader loads GENOME.md. There is no fallback to SOUL.md.

2. **IDENTITY.md and USER.md are auto-generated.** After every dream cycle, the dream engine extracts The Phenotype and writes it to IDENTITY.md, and extracts The Bond and writes it to USER.md. Both files include an autogeneration header:
   ```
   <!-- AUTOGENERATED BY DREAM ENGINE -- edits will be overwritten. To make permanent changes, edit GENOME.md -->
   ```

3. **Existing file loaders still work.** The bootstrap pipeline that reads IDENTITY.md and USER.md on session start continues to function. It now reads dream-generated content instead of static templates, but the loading mechanism is unchanged.

4. **Workspace initialization seeds the new file set.** New workspaces get: GENOME.md, PROTOCOLS.md, TOOLS.md, IDENTITY.md, USER.md, HEARTBEAT.md, MEMORY.md. SOUL.md and AGENTS.md are no longer created.

### Migration Path for Existing Users

If you have a workspace with a legacy SOUL.md, IDENTITY.md, or USER.md:
...
2. **Let the dream engine take over.** Within 1-2 dream cycles, the Phenotype and Bond sections will populate in MEMORY.md. The system will begin auto-generating IDENTITY.md and USER.md from dream output.

3. **Stop editing IDENTITY.md and USER.md manually.** Edits will be overwritten on the next dream cycle. Put personality constraints in GENOME.md instead.

4. **Delete SOUL.md.** It is no longer loaded by any part of the system. You can keep it in version history for reference.
