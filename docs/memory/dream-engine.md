...
Deferred within Phase 4 (noted as a follow-up): the one-time backfill of pre-existing `dream_insights` (dead-on-arrival until historical chunks carry `session_trust`, so it is not shipped auto-running). The promotion + proactive-recall surfacing already delivers "dream insights resurface, clearly marked as hunches".
