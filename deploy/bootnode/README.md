...
- Optional: a **project-scoped** Railway token for scripted work — see _Using a project token_ below.
...
**Important**: the `[[deploy.volumes]]` stanza in `railway.toml` does **not** actually create the volume. It only _binds_ an existing volume by name at deploy time. You must create the volume separately.
...
3. **Target Port**: `9100` (the internal port the orchestrator listens on — _not_ the external port Railway assigns)
4. Confirm
...
| ------------ | --------------------------------------------------------------------------------------- |
| Type         | `TXT`                                                                                   |
| Name         | `_dnsaddr.p2p` _(Cloudflare auto-appends the zone, final: `_dnsaddr.p2p.bitterbot.ai`)_ |
...
**Append, don't replace.** `_dnsaddr.<zone>` supports multiple TXT records at the same name — one per bootstrap peer. libp2p's DNS resolver reads all of them and tries each in turn. If you're adding a second or third bootnode, create a _new_ record alongside the existing ones; don't overwrite.
