...
function normalizeDenyList(cfg: BitterbotPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: BitterbotPluginApi["config"]): string[] {
...
export default function register(api: BitterbotPluginApi) {
  ...
  const timerService: BitterbotPluginService = {
